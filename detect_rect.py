...
def get_new_user_directory():
    """
    在当前目录下创建 upload_history 文件夹，并在该文件夹内创建一个新的子文件夹，
    名称为 1, 2, 3, 4, …（基于已有数字子文件夹自动递增）。
    在新的子文件夹内同时创建以下目录：
      - uploads：上传图片保存目录
...
if __name__ == "__main__":
    # 为当前用户创建一个独立的目录结构（目录名为 1, 2, 3, …），包含 uploads、mask_output、results、generated_ppt 四个子文件夹
    uploads_dir, mask_dir, output_dir, ppt_dir = get_new_user_directory()
    
    print("为当前用户创建的独立目录结构：")
    print("上传目录:", uploads_dir)
    print("mask 保存目录:", mask_dir)
    print("裁剪输出目录:", output_dir)
    print("PPT 存储目录:", ppt_dir)
    
    # 此处假设用户已经将图片上传至 uploads_dir 中；
    # 若为 Web 应用，则上传后使用该目录，不需要额外输入路径
    process_images(uploads_dir, mask_dir, output_dir)
    
    # 定义 PPT 文件输出路径，保存在 ppt_dir 内
    ppt_file_path = os.path.join(ppt_dir, "output.pptx")
    run_create_ppt(output_dir, ppt_file_path)
